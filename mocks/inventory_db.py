...
All SKU definitions, base pricing, and warehouse stock levels live here.
Supplier mocks and inventory agents import from this module — single source of truth.
"""
...
class WarehouseStock(BaseModel):
    """Current stock snapshot in the central warehouse."""

    sku: str
    on_hand_qty: int = Field(ge=0)
    reserved_qty: int = Field(ge=0)
    reorder_point: int = Field(ge=0, description="Trigger replenishment below this")
...
def get_all_skus() -> list[str]:
    """Return all known SKU codes."""
    return list(SKU_CATALOG.keys())


def get_sku(sku: str) -> SKURecord | None:
    """Look up a single SKU record, or None if not found."""
    return SKU_CATALOG.get(sku)


def get_stock(sku: str) -> WarehouseStock | None:
    """Look up current warehouse stock for a SKU."""
...
def get_critical_items() -> list[SKURecord]:
    """Return all SKUs flagged as critical/safety parts."""
